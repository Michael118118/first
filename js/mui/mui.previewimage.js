(function($, window) {
	...
	proto.initEvent = function() {
		...
		this.scroller.addEventListener('longtap', function() {						
			plus.nativeUI.actionSheet( {title:"选择操作",cancel:"取消",buttons:[{title:"保存到相册"}]}, function(e){										
					if(e.index == 1){ 
						if(current_src != ''){
							self.downloaderandsave(current_src);
						}
					}
			} );
	...
	//下载并保存图片
	proto.downloaderandsave = function(url_value){
